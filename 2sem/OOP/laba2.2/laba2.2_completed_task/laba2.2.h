...
public: 
    ...
    //Прототип оператора копирования
    //Позвляет скопировать один объект в другой
    void operator=(const List<T>& other);

    //Прототип оператора перемещения
    //Позволяет переместить один объект в другой
...
void List<T>::push_back(T data){
    ...
    //которому сразу же присваем значение головы
        ...
        //Есть ещё другой способ, например если хотим сразу хотим вставлять за O(1).
        //Тогда надо добавить указатель на "хвост" списка (Node *tail)
        //Ниже приведена реализация за O(n)
